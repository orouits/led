...
bool led_init_opt(led_str_t* arg) {
    bool rc = led_u8str_match_pat(arg, "^-[a-zA-Z]+");
    if (rc) {
        led_debug("arg option: %s", led_u8str_str(arg));
        size_t opti = 1;
        while(opti < arg->len) {
            u8chr_t opt = led_u8str_char_at_next(arg, &opti);
            char* optstr = led_u8str_str_at(arg, opti);
            ...
            case 'W':
                ...
                led_u8str_init_str(&led.opt.file_out_path, optstr);
                led_debug("Option path: %s", led_u8str_str(&led.opt.file_out_path));
                opti = arg->len;
                break;
            case 'A':
                led_assert(!led.opt.file_out, LED_ERR_ARG, "Bad option -%c, output file mode already set", opt);
                ...
                led_u8str_init(&led.opt.file_out_path, optstr, 0);
                led_debug("Option path: %s", led_u8str_str(&led.opt.file_out_path));
                opti = arg->len;
                break;
            case 'E':
                ...
                if (led.opt.file_out_extn <= 0)
                    led_u8str_init(&led.opt.file_out_ext, optstr, 0);
                led_debug("Option ext: %s", led_u8str_str(&led.opt.file_out_ext));
                opti =arg->len;
                break;
            case 'D':
                led_assert(!led.opt.file_out, LED_ERR_ARG, "Bad option -%c, output file mode already set", opt);
                led_assert(!led.opt.exec, LED_ERR_ARG, "Bad option -%c, exec mode already set", opt);
                led_u8str_init(&led.opt.file_out_dir, optstr, 0);
                led.opt.file_out = LED_OUTPUT_FILE_DIR;
                led_debug("Option dir: %s", led_u8str_str(&led.opt.file_out_dir));
                opti = arg->len;
                break;
            case 'U':
...
bool led_init_func(led_str_t* arg) {
    bool is_func = false;
    char fsep ='\0';
    if ( (is_func = led_u8str_match(arg, LED_REGEX_FUNC)) ) fsep = '/';
    else if ( (is_func = led_u8str_match(arg, LED_REGEX_FUNC2)) ) fsep = ':';
    ...
    if (is_func) {
        ...
        // search for function
        led_str_t fname;
        led_u8str_cut_next(arg, fsep, &fname);
        ...
        for (pfunc->id = 0; pfunc->id < led_fn_table_size(); pfunc->id++) {
            ...
            if (led_u8str_equal_str(&fname, pfn_desc->short_name) || led_u8str_equal_str(&fname, pfn_desc->long_name)) {
        ...
        led_assert(pfunc->id < led_fn_table_size(), LED_ERR_ARG, "Unknown function: %s", led_u8str_str(&fname));
        ...
        led_str_t regx;
        led_u8str_cut_next(arg, fsep, &regx);
        if (!led_u8str_isempty(&regx)) {
            led_debug("Regex found: %s", led_u8str_str(&regx));
            pfunc->regex = led_u8str_regex_compile(&regx);
        }
        else {
        ...
        while(!led_u8str_isempty(arg)) {
            ...
            led_u8str_cut_next(arg, fsep, farg);
            led_debug("Function argument found: %s", led_u8str_str(farg));
        }
    }
    return is_func;
}
...
bool led_init_sel(led_str_t* arg) {
    bool rc = true;
    if (led_u8str_match_pat(arg, "^\\+[0-9]+$") && led.sel.type_start == SEL_TYPE_REGEX) {
        ...
        if (led_u8str_match(arg, LED_REGEX_INTEGER)) {
            ...
            led.sel.regex_start = led_u8str_regex_compile(arg);
            led_debug("Selector start: type regex (%s)", led_u8str_str(arg));
        }
    }
        ...
        if (led_u8str_match(arg, LED_REGEX_INTEGER)) {
            ...
            led.sel.regex_stop = led_u8str_regex_compile(arg);
            led_debug("Selector stop: type regex (%s)", led_u8str_str(arg));
        }
    }
    else rc = false;
...
void led_init_config() {
    for (size_t ifunc = 0; ifunc < led.func_count; ifunc++) {
        ...
        for (size_t i=0; format[i] && i < LED_FARG_MAX; i++) {
            if (format[i] == 'R') {
                led_assert(led_u8str_isinit(&pfunc->arg[i].lstr), LED_ERR_ARG, "function arg %i: missing regex\n%s", i+1, pfn_desc->help_format);
                pfunc->arg[i].regex = led_u8str_regex_compile(&pfunc->arg[i].lstr);
                led_debug("function arg %i: regex found", i+1);
            }
            else if (format[i] == 'r') {
                if (led_u8str_isinit(&pfunc->arg[i].lstr)) {
                    pfunc->arg[i].regex = led_u8str_regex_compile(&pfunc->arg[i].lstr);
                    led_debug("function arg %i: regex found", i+1);
                }
            }
            else if (format[i] == 'N') {
                led_assert(led_u8str_isinit(&pfunc->arg[i].lstr), LED_ERR_ARG, "function arg %i: missing number\n%s", i+1, pfn_desc->help_format);
                pfunc->arg[i].val = atol(led_u8str_str(&pfunc->arg[i].lstr));
                ...
                led_assert(led_u8str_isinit(&pfunc->arg[i].lstr), LED_ERR_ARG, "function arg %i: missing string\n%s", i+1, pfn_desc->help_format);
                led_debug("function arg %i: string found: %s", i+1, led_u8str_str(&pfunc->arg[i].lstr));
            }
            else if (format[i] == 's') {
                if (led_u8str_isinit(&pfunc->arg[i].lstr)) {
                    led_debug("function arg %i: string found: %s", i+1, led_u8str_str(&pfunc->arg[i].lstr));
...
void led_init(int argc, char* argv[]) {
    ...
    for (int argi=1; argi < argc; argi++) {
        led_u8str_decl_str(arg, argv[argi]);

        if (arg_section == ARGS_SEC_FILES) {
            led.file_names = argv + argi;
            led.file_count = argc - argi;
            led_debug("Arg is file: %s", led_u8str_str(&arg));
        }
        else if (arg_section < ARGS_SEC_FILES && led_init_opt(&arg)) {
            if (led.opt.file_in) arg_section = ARGS_SEC_FILES;
            led_debug("Arg is opt: %s", led_u8str_str(&arg));
        }
        else if (arg_section <= ARGS_SEC_FUNCT && led_init_func(&arg)) {
            arg_section = ARGS_SEC_FUNCT;
            led_debug("Arg is func: %s", led_u8str_str(&arg));
        }
        else if (arg_section == ARGS_SEC_SELECT && led_init_sel(&arg)) {
            led_debug("Arg is part of selector: %s", led_u8str_str(&arg));
        }
        else {
            led_assert(false, LED_ERR_ARG, "Unknown or wrong argument: %s (%s section)", led_u8str_str(&arg), LED_SEC_TABLE[arg_section]);
        }
    }
    ...
    led_u8str_init_buf(&led.file_in.name, led.file_in.buf_name);
    led_u8str_init_buf(&led.file_out.name, led.file_out.buf_name);
...
void led_file_open_in() {
    ...
    if (led.file_count) {
        led_u8str_cpy_chars(&led.file_in.name, led.file_names[0]);
        led.file_names++;
        led.file_count--;
        led_str_trim(&led.file_in.name);
        led_debug("open file from args: %s", led_u8str_str(&led.file_in.name));
        led.file_in.file = fopen(led_u8str_str(&led.file_in.name), "r");
        led_assert(led.file_in.file != NULL, LED_ERR_FILE, "File not found: %s", led_u8str_str(&led.file_in.name));
        ...
        if (fname) {
            led_u8str_cpy_chars(&led.file_in.name, fname);
            led_str_trim(&led.file_in.name);
            led_debug("open file from stdin: [%s]", led_u8str_str(&led.file_in.name));
            led.file_in.file = fopen(led_u8str_str(&led.file_in.name), "r");
            led_assert(led.file_in.file != NULL, LED_ERR_FILE, "File not found: %s", led_u8str_str(&led.file_in.name));
            led.report.file_in_count++;
        }
...
void led_file_stdin() {
    if (led.file_in.file) {
        led_assert(led.file_in.file == stdin, LED_ERR_FILE, "File is not STDIN internal error: %s", led_u8str_str(&led.file_in.name));
        led.file_in.file = NULL;
        led_u8str_empty(&led.file_in.name);
    } else if (led.stdin_ispipe) {
        led.file_in.file = stdin;
        led_u8str_cpy_chars(&led.file_in.name, "STDIN");
    }
}
...
void led_file_open_out() {
    ...
    if (led.opt.file_out == LED_OUTPUT_FILE_INPLACE) {
        led_u8str_cpy(&led.file_out.name, &led.file_in.name);
        led_u8str_app_str(&led.file_out.name, ".part");
        mode = "w+";
    }
    else if (led.opt.file_out == LED_OUTPUT_FILE_WRITE) {
        led_u8str_cpy(&led.file_out.name, &led.opt.file_out_path);
        mode = "w+";
    }
        ...
        led_u8str_cpy(&led.file_out.name, &led.file_in.name);
        led_u8str_app(&led.file_out.name, &led.opt.file_out_ext);
        mode = "w+";
    }
    else if (led.opt.file_out == LED_OUTPUT_FILE_DIR) {
        led_u8str_decl(tmp, LED_FNAME_MAX+1);

        led_u8str_cpy(&led.file_out.name, &led.opt.file_out_dir);
        led_u8str_app_str(&led.file_out.name, "/");
        led_u8str_cpy(&tmp, &led.file_in.name);
        led_u8str_app(&led.file_out.name, led_u8str_basename(&tmp));
        mode = "w+";
    }
    led.file_out.file = fopen(led_u8str_str(&led.file_out.name), mode);
    led_assert(led.file_out.file != NULL, LED_ERR_FILE, "File open error: %s", led_u8str_str(&led.file_out.name));
...
void led_file_close_out() {
    ...
    led.file_out.file = NULL;
    if (led.opt.file_out == LED_OUTPUT_FILE_INPLACE) {
        led_u8str_cpy(&tmp, &led.file_out.name);
        led_str_trunk_end(&tmp, 5);
        led_debug("Rename: %s ==> %s", led_u8str_str(&led.file_out.name), led_u8str_str(&tmp));
        int syserr = remove(led_u8str_str(&tmp));
        led_assert(!syserr, LED_ERR_FILE, "File remove error: %d => %s", syserr, led_u8str_str(&tmp));
        rename(led_u8str_str(&led.file_out.name), led_u8str_str(&tmp));
        led_assert(!syserr, LED_ERR_FILE, "File rename error: %d => %s", syserr, led_u8str_str(&led.file_out.name));
        led_u8str_cpy(&led.file_out.name, &tmp);
    }
}

void led_file_print_out() {
    fwrite(led_u8str_str(&led.file_out.name), sizeof *led_u8str_str(&led.file_out.name), led_u8str_len(&led.file_out.name), stdout);
    fwrite("\n", sizeof *led_u8str_str(&led.file_out.name), 1, stdout);
    fflush(stdout);
    led_u8str_empty(&led.file_out.name);
}
...
void led_file_stdout() {
    ...
    led_u8str_cpy_chars(&led.file_out.name, "STDOUT");
}
...
bool led_file_next() {
    ...
    led_debug("Input from: %s", led_u8str_str(&led.file_in.name));
    led_debug("Output to: %s", led_u8str_str(&led.file_out.name));
...
bool led_process_read() {
    ...
    if (!led_line_isinit(&led.line_read)) {
        led_u8str_init(&led.line_read.lstr, fgets(led.line_read.buf, sizeof led.line_read.buf, led.file_in.file), sizeof led.line_read.buf);
        if (led_line_isinit(&led.line_read)) {
...
void led_process_write() {
    ...
    if (led_line_isinit(&led.line_write)) {
        led_debug("Write line: (%d) len=%d", led.sel.total_count, led_u8str_len(&led.line_write.lstr));
        led_u8str_app_char(&led.line_write.lstr, '\n');
        led_debug("Write line to %s", led_u8str_str(&led.file_out.name));
        fwrite(led_u8str_str(&led.line_write.lstr), sizeof *led_u8str_str(&led.line_write.lstr), led_u8str_len(&led.line_write.lstr), led.file_out.file);
...
void led_process_exec() {
    ...
    if (led_line_isinit(&led.line_write) && !led_u8str_isblank(&led.line_write.lstr)) {
        led_debug("Exec line: (%d) len=%d", led.sel.total_count, led_u8str_len(&led.line_write.lstr));
        led_debug("Exec command %s", led_u8str_str(&led.line_write.lstr));

        FILE *fp = popen(led_u8str_str(&led.line_write.lstr), "r");
        led_assert(fp != NULL, LED_ERR_ARG, "Command error");
        led_u8str_decl(output, 4096);
        while (led_u8str_isinit(led_u8str_init(&output, fgets(led_u8str_str(&output), led_u8str_size(&output), fp), led_u8str_size(&output)))) {
            fwrite(led_u8str_str(&output), sizeof *led_u8str_str(&output), led_u8str_len(&output), led.file_out.file);
            fflush(led.file_out.file);
        }
        pclose(fp);
    }
...
bool led_process_selector() {
    ...
    if (!led_line_isinit(&led.line_read)
        ...
        || (led.sel.type_stop == SEL_TYPE_REGEX && led_u8str_match(&led.line_read.lstr, led.sel.regex_stop))
        ) {
        led.sel.inboundary = false;
        led.sel.count = 0;
    }
    ...
    if (led_line_isinit(&led.line_read) && (
        ...
        || (led.sel.type_start == SEL_TYPE_REGEX && led_u8str_match(&led.line_read.lstr, led.sel.regex_start))
        )) {
    ...
    if (led.opt.pack_selected) {
        if (led_line_isselected(&led.line_read)) {
            ...
            if (!(led.opt.filter_blank && led_u8str_isblank(&led.line_read.lstr))) {
                if (led_u8str_iscontent(&led.line_prep.lstr))
                    led_u8str_app_char(&led.line_prep.lstr, '\n');
                led_u8str_app(&led.line_prep.lstr, &led.line_read.lstr);
            }
...
void led_process_functions() {
    ...
    led_debug("Process line prep (isinit: %d len: %d)", led_line_isinit(&led.line_prep), led_u8str_len(&led.line_prep.lstr));
    if (led_line_isinit(&led.line_prep)) {
        ...
        if (led_line_isselected(&led.line_prep)) {
            ...
            if (led.func_count > 0) {
                ...
                led_debug("No function copy (len: %d)", led_u8str_len(&led.line_prep.lstr));
    ...
    led_debug("Process result line write (len=%d)", led_u8str_len(&led.line_write.lstr));
    led_line_reset(&led.line_prep);
}
