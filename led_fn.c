...
bool led_zone_pre_process(led_fn_t* pfunc) {
    ...
    led.line_prep.zone_start = led.line_prep.zone_stop = led_u8str_len(&led.line_prep.lstr);
    bool rc = led_u8str_match_offset(&led.line_prep.lstr, pfunc->regex, &led.line_prep.zone_start, &led.line_prep.zone_stop);

    if (!led.opt.output_match)
        led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, 0, led.line_prep.zone_start);
...
void led_zone_post_process() {
    if (!led.opt.output_match)
        led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, led.line_prep.zone_stop, led.line_prep.lstr.len);
}
...
void led_fn_impl_register(led_fn_t* pfunc) {
    ...
    int rc = pcre2_match(pfunc->regex, (PCRE2_SPTR)led_u8str_str(&led.line_prep.lstr), led_u8str_len(&led.line_prep.lstr), 0, 0, match_data, NULL);
    ...
    if (pfunc->arg_count > 0) {
        ...
        if( rc > 0) {
            ...
            led_u8str_app_start_stop(&led.line_reg[ir].lstr, &led.line_prep.lstr, ovector[iv], ovector[iv+1]);
            led_debug("register value %d (%s)", ir, led_u8str_str(&led.line_reg[ir].lstr));
        }
    }
    else {
...
void led_fn_impl_register_recall(led_fn_t* pfunc) {
    ...
    if (led_line_isinit(&led.line_reg[ir])) {
        led.line_reg[ir].zone_start = led.line_reg[ir].zone_stop = led_u8str_len(&led.line_reg[ir].lstr);
        led_u8str_match_offset(&led.line_reg[ir].lstr, pfunc->regex, &led.line_reg[ir].zone_start, &led.line_reg[ir].zone_stop);
        led_line_init(&led.line_write);
        led_u8str_app_start_stop(&led.line_write.lstr, &led.line_reg[ir].lstr, led.line_reg[ir].zone_start, led.line_reg[ir].zone_stop);
    }
    else {
...
void led_fn_helper_substitute(led_fn_t* pfunc, led_str_t* sinput, led_str_t* soutput) {
    ...
    led_debug("Substitute input line (len=%d) to rsval (len=%d)", led_u8str_len(sinput), led_u8str_len(&rsval));
    PCRE2_SIZE len = led_u8str_size(soutput);
    int rc = pcre2_substitute(
                pfunc->regex,
                (PCRE2_UCHAR8*)led_u8str_str(sinput),
                led_u8str_len(sinput),
                0,
                PCRE2_SUBSTITUTE_EXTENDED|PCRE2_SUBSTITUTE_GLOBAL,
                NULL,
                NULL,
                (PCRE2_UCHAR8*)led_u8str_str(&rsval),
                led_u8str_len(&rsval),
                (PCRE2_UCHAR8*)led_u8str_str(soutput),
                &len);
    led_assert_pcre(rc);
    soutput->len = len;
}
...
void led_fn_impl_substitute(led_fn_t* pfunc) {
    led_fn_helper_substitute(pfunc, &led.line_prep.lstr, led_u8str_init_buf(&led.line_write.lstr,led.line_write.buf));
}

void led_fn_impl_delete(led_fn_t* pfunc) {
    led_zone_pre_process(pfunc);

    if (led.line_prep.zone_start == 0 && led.line_prep.zone_stop == led_u8str_len(&led.line_prep.lstr))
...
void led_fn_impl_delete_blank(led_fn_t*) {
    if (led_u8str_isempty(&led.line_prep.lstr) || led_u8str_isblank(&led.line_prep.lstr))
        led_line_reset(&led.line_write);
    else
        led_u8str_cpy(&led.line_write.lstr, &led.line_prep.lstr);
}

void led_fn_impl_insert(led_fn_t* pfunc) {
    led_u8str_decl(newline, LED_BUF_MAX);
    led_fn_helper_substitute(pfunc, &led.line_prep.lstr, &newline);

    led_u8str_init_buf(&led.line_write.lstr, led.line_write.buf);
    led_u8str_empty(&led.line_write.lstr);
    size_t lcount = pfunc->arg_count > 1 ? pfunc->arg[1].uval : 1;
    for (size_t i = 0; i < lcount; i++) {
        led_u8str_app(&led.line_write.lstr, &newline);
        led_u8str_app_char(&led.line_write.lstr, '\n');
    }
    led_u8str_app(&led.line_write.lstr, &led.line_prep.lstr);
}
...
void led_fn_impl_range_sel(led_fn_t* pfunc) {
    ...
    if (led_u8str_iscontent(&pfunc->arg[0].lstr)) {
        long val = pfunc->arg[0].val;
        size_t uval = pfunc->arg[0].uval;
        if (val > 0)
            led.line_prep.zone_start = uval > led_u8str_len(&led.line_prep.lstr) ? led_u8str_len(&led.line_prep.lstr) : uval;
        else
            led.line_prep.zone_start = uval > led_u8str_len(&led.line_prep.lstr) ? 0 : led_u8str_len(&led.line_prep.lstr) - uval;
    }
    if (led_u8str_iscontent(&pfunc->arg[1].lstr)) {
        size_t uval = pfunc->arg[1].uval;
        led.line_prep.zone_stop = led.line_prep.zone_start + uval > led_u8str_len(&led.line_prep.lstr) ? led_u8str_len(&led.line_prep.lstr) : led.line_prep.zone_start + uval;
    }
    else
        led.line_prep.zone_stop = led_u8str_len(&led.line_prep.lstr);
...
void led_fn_impl_translate(led_fn_t* pfunc) {
    ...
    while ( i < led.line_prep.zone_stop ) {
        ...
        size_t j=0,k=0;
        while ( j < led_u8str_len(&pfunc->arg[0].lstr) && k < led_u8str_len(&pfunc->arg[1].lstr) ) {
            u8chr_t ct = led_u8str_char_at_next(&pfunc->arg[1].lstr, &k);
            if (led_u8str_char_at_next(&pfunc->arg[0].lstr, &j) == c) {
                led_u8str_app_char(&led.line_write.lstr, ct);
                break;
            }
        }
        // if no substitution has been done
        if (j == led_u8str_len(&pfunc->arg[0].lstr))
...
void led_fn_impl_case_lower(led_fn_t* pfunc) {
    ...
    for (size_t i=led.line_prep.zone_start; i<led.line_prep.zone_stop; i++)
        led_u8str_app_char(&led.line_write.lstr, tolower(led_u8str_char_at(&led.line_prep.lstr, i)));
...
void led_fn_impl_case_upper(led_fn_t* pfunc) {
    ...
    for (size_t i=led.line_prep.zone_start; i<led.line_prep.zone_stop; i++)
        led_u8str_app_char(&led.line_write.lstr, toupper(led_u8str_char_at(&led.line_prep.lstr, i)));
...
void led_fn_impl_case_first(led_fn_t* pfunc) {
    ...
    led_u8str_app_char(&led.line_write.lstr, toupper(led_u8str_char_at(&led.line_prep.lstr, led.line_prep.zone_start)));
...
void led_fn_impl_case_camel(led_fn_t* pfunc) {
    ...
    bool wasword = false;
    for (size_t i=led.line_prep.zone_start; i<led.line_prep.zone_stop; i++) {
        int c = led_u8str_char_at(&led.line_prep.lstr, i);
        bool isword = isalnum(c) || c == '_';
        if (isword) {
            if (wasword) led_u8str_app_char(&led.line_write.lstr, tolower(c));
            else led_u8str_app_char(&led.line_write.lstr, toupper(c));
        }
        wasword = isword;
    }
...
void led_fn_impl_case_snake(led_fn_t* pfunc) {
    ...
    for (size_t i=led.line_prep.zone_start; i<led.line_prep.zone_stop; i++) {
        ...
        char lc = led_u8str_char_last(&led.line_write.lstr);
        if (isalnum(c))
            led_u8str_app_char(&led.line_write.lstr, tolower(c));
        else if (lc != '_')
            led_u8str_app_char(&led.line_write.lstr, '_');
    }

    led_zone_post_process();
}

void led_fn_impl_quote_base(led_fn_t* pfunc, u8chr_t q) {
    led_zone_pre_process(pfunc);

    if (! (led_u8str_char_at(&led.line_prep.lstr, led.line_prep.zone_start) == q && led_u8str_char_at(&led.line_prep.lstr, led.line_prep.zone_stop - 1) == q) ) {
        led_debug("quote active");
        led_u8str_app_char(&led.line_write.lstr, q);
        led_line_append_zone(&led.line_write, &led.line_prep);
        led_u8str_app_char(&led.line_write.lstr, q);
    }
    else
...
void led_fn_impl_quote_remove(led_fn_t* pfunc) {
    ...
    for(size_t i = 0; q != '\0'; i++, q = QUOTES[i]) {
        if (led_u8str_char_at(&led.line_prep.lstr, led.line_prep.zone_start) == (u8chr_t)q && led_u8str_char_at(&led.line_prep.lstr, led.line_prep.zone_stop - 1) == (u8chr_t)q) break;
    }

    if (q) {
        led_debug("quotes found: %c", q);
        led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, led.line_prep.zone_start + 1, led.line_prep.zone_stop - 1);
    }
    else
        led_line_append_zone(&led.line_write, &led.line_prep);
...
void led_fn_impl_trim(led_fn_t* pfunc) {
    ...
    for (; str_start < led.line_prep.zone_stop; str_start++) {
        if (!isspace(led_u8str_char_at(&led.line_prep.lstr, str_start))) break;
    }
    for (; str_stop > str_start; str_stop--) {
        if (!isspace(led_u8str_char_at(&led.line_prep.lstr, str_stop - 1))) break;
    }
    led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, str_start, str_stop);
...
void led_fn_impl_trim_left(led_fn_t* pfunc) {
    ...
    led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, str_start, led.line_prep.zone_stop);
...
void led_fn_impl_trim_right(led_fn_t* pfunc) {
    ...
    for (; str_stop > led.line_prep.zone_start; str_stop--)
        if (!isspace(led_u8str_char_at(&led.line_prep.lstr, str_stop - 1))) break;
    led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, led.line_prep.zone_start, str_stop);
...
void led_fn_impl_base64_encode(led_fn_t* pfunc) {
	...
	count = base64_encode_block(
        led_u8str_str_at(&led.line_prep.lstr, led.line_prep.zone_start),
    ...
    led_u8str_app_str(&led.line_write.lstr, b64buf);
    led_zone_post_process();
}
...
void led_fn_impl_url_encode(led_fn_t* pfunc) {
    ...
    while ( i < led.line_prep.zone_stop ) {
        ...
        if (led_u8chr_isalnum(c))
            ...
            pcbuf[2] = HEX[c & 0x0F];
            led_u8str_app_str(&led.line_write.lstr, pcbuf);
        }
    }
...
void led_fn_impl_shell_escape(led_fn_t* pfunc) {
    ...
    led_u8str_decl_str(table, fname_stdchar_table);

    led_zone_pre_process(pfunc);
    size_t i = led.line_prep.zone_start;
    while ( i < led.line_prep.zone_stop ) {
        u8chr_t c = led_u8str_char_at_next(&led.line_prep.lstr, &i);
        if (led_u8chr_isalnum(c) || led_u8str_ischar(&table, c))
            led_u8str_app_char(&led.line_write.lstr, c);
        else {
            led_u8str_app_char(&led.line_write.lstr, '\\');
            led_u8str_app_char(&led.line_write.lstr, c);
...
void led_fn_impl_realpath(led_fn_t* pfunc) {
    ...
    if (realpath(led_u8str_str_at(&led.line_prep.lstr, led.line_prep.zone_start), led.line_write.buf + led_u8str_len(&led.line_write.lstr)) != NULL ) {
...
void led_fn_impl_dirname(led_fn_t* pfunc) {
    ...
    const char* dir = dirname(led_u8str_str_at(&led.line_prep.lstr, led.line_prep.zone_start));
    if (dir != NULL) led_u8str_app_str(&led.line_write.lstr, dir);
...
void led_fn_impl_basename(led_fn_t* pfunc) {
    ...
    const char* fname = basename(led_u8str_str_at(&led.line_prep.lstr, led.line_prep.zone_start));
    if (fname != NULL) led_u8str_app_str(&led.line_write.lstr, fname);
    else led_line_append_zone(&led.line_write, &led.line_prep);
...
void led_fn_impl_revert(led_fn_t* pfunc) {
    ...
    for (size_t i = led.line_prep.zone_stop; i > led.line_prep.zone_start; i--)
        led_u8str_app_char(&led.line_write.lstr, led_u8str_char_at(&led.line_prep.lstr, i - 1));

    led_zone_post_process();
}

void led_fn_impl_field_base(led_fn_t* pfunc, const char* field_sep) {
    led_u8str_decl_str(sepsval, field_sep);
    led_zone_pre_process(pfunc);
    ...
    size_t str_stop = led.line_prep.zone_stop;
    for (; str_start < led.line_prep.zone_stop; str_start++ ) {
        bool is_sep = led_u8str_ischar(&sepsval, led_u8str_char_at(&led.line_prep.lstr, str_start));
        if (was_sep && !is_sep) {
            n++;
            if (n == field_n) break;
        }
    ...
    if (n == field_n) {
        ...
        for (str_stop = str_start; str_stop < led.line_prep.zone_stop; str_stop++ ) {
            bool is_sep = led_u8str_ischar(&sepsval, led_u8str_char_at(&led.line_prep.lstr, str_stop));
...
void led_fn_impl_field(led_fn_t* pfunc) { led_fn_impl_field_base(pfunc, led_u8str_str(&pfunc->arg[1].lstr)); }
...
void led_fn_impl_join(led_fn_t*) {
   for (size_t i = 0; i < led_u8str_len(&led.line_prep.lstr); i++) {
        char c = led_u8str_char_at(&led.line_prep.lstr, i);
        if ( c != '\n') led_u8str_app_char(&led.line_write.lstr, c);
   }
}
...
void led_fn_impl_split_base(led_fn_t* pfunc, const char* field_sep) {
    ...
    for (size_t i = led.line_prep.zone_start; i < led.line_prep.zone_stop; i++) {
        char c = led_u8str_char_at(&led.line_prep.lstr, i);
        if ( led_u8str_ischar(&sepsval, c) ) c = '\n';
        led_u8str_app_char(&led.line_write.lstr, c);
    }
    led_zone_post_process();
}

void led_fn_impl_split(led_fn_t* pfunc) { led_fn_impl_split_base(pfunc, led_u8str_str(&pfunc->arg[0].lstr)); }
...
size_t led_fn_helper_fname_pos() {
    size_t iname = led_u8str_rfind_char_start_stop(&led.line_prep.lstr, '/', led.line_prep.zone_start, led.line_prep.zone_stop);
    if (iname == led_u8str_len(&led.line_prep.lstr)) iname = led.line_prep.zone_start;
    else iname++;
    led_debug("led_fn_helper_fname_pos iname: %u %s", iname, led_u8str_str_at(&led.line_prep.lstr, iname));
    return iname;
}
...
void led_fn_impl_fname_lower(led_fn_t* pfunc) {
    ...
    if (led.line_prep.zone_start < led.line_prep.zone_stop) {
        ...
        led_u8str_app_start_stop(&led.line_write.lstr, &led.line_prep.lstr, led.line_prep.zone_start, iname);

        for (; iname < led.line_prep.zone_stop; iname++) {
            char c = led_u8str_char_at(&led.line_prep.lstr, iname);
            if (isalnum(c))
                led_u8str_app_char(&led.line_write.lstr, tolower(c));
            else if (c == '.') {
                if (!isalnum(led_u8str_char_last(&led.line_write.lstr)))
                    led_str_trunk_char_last(&led.line_write.lstr);
                led_u8str_app_char(&led.line_write.lstr, c);
            }
            else {
                if (isalnum(led_u8str_char_last(&led.line_write.lstr))) led_u8str_app_char(&led.line_write.lstr, '_');
            }
        }
    }
    led_zone_post_process();
...
void led_fn_impl_fname_upper(led_fn_t* pfunc) {
    ...
    if (led.line_prep.zone_start < led.line_prep.zone_stop) {
        ...
        for (; iname < led.line_prep.zone_stop; iname++) {
            char c = led_u8str_char_at(&led.line_prep.lstr, iname);
            if (isalnum(c))
                led_u8str_app_char(&led.line_write.lstr, toupper(c));
            else if (c == '.') {
                if (!isalnum(led_u8str_char_last(&led.line_write.lstr)))
                    led_str_trunk_char_last(&led.line_write.lstr);
                led_u8str_app_char(&led.line_write.lstr, c);
...
void led_fn_impl_fname_camel(led_fn_t* pfunc) {
    ...
    if (led.line_prep.zone_start < led.line_prep.zone_stop) {
        ...
        for (; iname < led.line_prep.zone_stop; iname++) {
            char c = led_u8str_char_at(&led.line_prep.lstr, iname);
            if (isalnum(led_u8str_char_last(&led.line_write.lstr)) && c == '.') {
                led_u8str_app_char(&led.line_write.lstr, c);
                isfirst = true;
            }
            else {
                ...
                if (isword) {
                    if (wasword || isfirst) led_u8str_app_char(&led.line_write.lstr, tolower(c));
                    else led_u8str_app_char(&led.line_write.lstr, toupper(c));
                    isfirst = false;
                }
                wasword = isword;
...
void led_fn_impl_generate(led_fn_t* pfunc) {
    ...
    char c = led_u8str_char_first(&pfunc->arg[0].lstr);
