...
int led_init_opt(led_str_t* arg) {
    int rc = led_str_match_pat(arg, "^-[a-zA-Z]+");
    if (rc) {
        led_debug("arg option: %s", led_str_str(arg));
        for(size_t opti = 1; opti < arg->len; opti++) {
            char opt = led_str_char_at(arg, opti);
            char* optstr = led_str_str_at(arg, opti + 1);
...
int led_init_func(led_str_t* arg) {
    int is_func = FALSE;
    char fsep ='\0';
...
int led_init_sel(led_str_t* arg) {
    int rc = TRUE;
...
int led_process_read() {
    ...
    if (!led_line_isinit(&led.line_read)) {
        ...
        if (led_line_isinit(&led.line_read)) {
            led_str_unapp_char(&led.line_read.lstr, '\n');
